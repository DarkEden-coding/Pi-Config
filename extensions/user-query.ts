...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      "Ask the user a series of questions. Each question has any number of pickable options plus a custom response choice.",
    promptSnippet:
      "Ask the user one or more multiple-choice questions with an optional custom response.",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      async function askQuestionsOverRpc(): Promise<{
        answers: Answer[];
        cancelled: boolean;
      }> {
        ...
        for (const [index, question] of params.questions.entries()) {
          const customOption = "Custom response…";
          const choice = await ctx.ui.select(
            `Question ${index + 1}/${params.questions.length}: ${question.question}`,
            [...question.options, customOption],
          );
      ...
      const result =
        ctx.mode === "rpc"
          ? await askQuestionsOverRpc()
          : await ctx.ui.custom<{ answers: Answer[]; cancelled: boolean }>(
              (tui, theme, _kb, done) => {
                let questionIndex = 0;
                let optionIndex = 0;
                let inputMode = false;
                ...
                function moveToQuestion(nextIndex: number) {
                  questionIndex = Math.max(
                    0,
                    Math.min(params.questions.length - 1, nextIndex),
                  );
                ...
                function render(width: number): string[] {
                  ...
                  add(
                    theme.fg(
                      "dim",
                      " ←→ previous/next question • ↑↓ select • Enter answer/submit last • Esc cancel",
                    ),
                  );
                  add(theme.fg("accent", "─".repeat(width)));
                  cachedWidth = width;
